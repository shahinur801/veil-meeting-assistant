...
const SYSTEM = `You are Veil, a live meeting assistant sitting privately on the user's screen. You never join the call. You help the user in the moment: what to say next, sharp follow-up questions, recaps, and structured notes.

Rules:
- Be concise, specific, and immediately usable.
- Ground every answer in the transcript. If the transcript is thin, say so and still be useful.
- Do not mention being an AI unless asked.
- Never invent attendees, numbers, or commitments that are not in the transcript.
- Write in a calm, professional tone.`;

function actionPrompt(action: AssistRequest["action"], question?: string) {
  switch (action) {
    case "say":
      return `Based on the live transcript, suggest 3 things the user could say next. Each should be a short spoken line they can read aloud (first person). Prefer the strongest commercial / interpersonal move. Format as a numbered list with a 4–8 word bold lead-in, then the line.`;
    case "followups":
  ...
  try {
    res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        contents: [{ role: "user", parts: [{ text: userPromptFor(data) }] }],
        generationConfig: {
          temperature: data.action === "notes" ? 0.3 : 0.6,
          maxOutputTokens: data.action === "notes" ? 2048 : 1024,
          ...(data.action === "notes"
  ...
  try {
    res = await fetch("https://api.x.ai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "grok-4.5",
        temperature: data.action === "notes" ? 0.3 : 0.6,
        max_tokens: data.action === "notes" ? 2048 : 1024,
        messages: [
          { role: "system", content: SYSTEM },
          { role: "user", content: userPromptFor(data) },
        ],
      }),
    });
  } catch {
...
export const runAssist = createServerFn({ method: "POST" })
  .validator((input: AssistRequest) => {
    ...
    if (!["say", "followups", "recap", "notes", "ask"].includes(action)) {
    ...
    return { apiKey, model, action, transcript, question } satisfies AssistRequest;
  })
